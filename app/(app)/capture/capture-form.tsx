...
export default function CaptureForm({ themes }: { themes: Theme[] }) {
  ...
  const activeTema = themes.find((t) => t.key === temaId);

  function platformsFor(key: string): Platform[] {
    return themes.find((t) => t.key === key)?.accounts.map((a) => a.platform) ?? [];
  }

  function applyTema(newTema: string, existing: ExecutionDraft[]) {
    return platformsFor(newTema).map((platform) => {
  ...
  async function handleParse() {
    ...
    try {
      ...
      setExecutions(
        platformsFor(parsed.tema).map((platform) => {
  ...
  return (
    <div className="space-y-6">
      <PageTitle
        title="Capture ide"
        subtitle="Tulis bebas kayak lagi cerita. Nggak usah rapi — AI yang beresin."
      />
        ...
        <div className="space-y-5 rounded-xl border border-hairline bg-surface p-5">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-2">
              {themes.map((t) => {
                const selected = t.key === temaId;
                return (
                  <button
                    key={t.key}
                    onClick={() => {
                      setTemaId(t.key);
                      setExecutions((prev) => applyTema(t.key, prev));
                    }}
